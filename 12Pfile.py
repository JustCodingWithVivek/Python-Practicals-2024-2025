    ...
    try:
        while True:
            ...
            if S[0]==no:
                ...
                pickle.dump(S,F)
                found=1
                F.seek(Pos) #moving the file pointer again beginning of the current record. print ("Mark updated Successfully and Details are:",S) break
                print("Mark updated successfully and details are:",S)
                break
    except:
        F.close()
    if found==0:
        print("The Seaaarched roll no is not found")
#Main Program
Create()
Update ()
